...
function wprmm_get_menu($atts){
  ...
  $m .= '<div class="wprmm_menu">'."\r\n";
  $m .= '<div class="menu_manager">'."\r\n";
  $m .= '<h1>'.$menu['name'].'</h1>'."\r\n";
  $m .= '<p class="menu_desc">'.$menu['description'].'</p>'."\r\n";
  $m .= '<div class="menu_manager">'."\r\n";
  foreach($categories as $cat) {
    if($cat->active){
      $m .= '<div id="'.wprmm_css_name($cat->name).'" class="menu_category">'."\r\n";
      if($cat->show_title) $m .= '<h2>'.$cat->name.'</h2>'."\r\n";
      if($cat->show_description) $m .= '<p class="menu_category_desc">'.$cat->description.'</p>'."\r\n";
